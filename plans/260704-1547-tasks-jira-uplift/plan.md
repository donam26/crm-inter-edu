...
### [~] Giai đoạn 2 (LÕI) — Thông báo & SLA ✅ · P2b hoãn
Chi tiết: [phase-02-notifications-sla.md](phase-02-notifications-sla.md)
**Lõi đã làm (nhánh `feat/tasks-work-item-depth`):** hệ thông báo (1 lớp `TaskNotification` in-app + email) · watcher + auto-watch (assign/comment) + watch/unwatch · `TaskObserver` bắn assigned/status_changed · bình luận bắn commented · **scheduler `tasks:dispatch-reminders`** (kích hoạt `remind_at` + cảnh báo quá hạn, chống trùng bằng cột `reminded_at`/`overdue_notified_at`) đăng ký `*/15` · chuông ở topbar + trang `/notifications` (mở/đọc-tất-cả) · chip lọc nhanh (Việc của tôi / Quá hạn / Hôm nay / Tôi theo dõi).
